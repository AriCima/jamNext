...
const InviteJammerForm = ({ roomNr }) => {
  ...
  const onSubmit = (data) => {
    ...
    if (outLater) {
      setDatesErrorType(`${dict.dates.invalidRange}`);
      setDatesErrorMessage(`${dict.dates.checkOutLater}`);
    ...
    for (let i = 0; i < roomJammers.length; i++) {
      ...
      if (isOverlapping) {
        ...
        setDatesErrorType(`${dict.dates.overlapping}`);
        setDatesErrorMessage(`${dict.common.tenant}: ${firstName} ${lastName}, ${dict.common.roomNr}: ${roomNr}, check-In: ${checkIn}, check-out: ${checkOut}`);
        setShowErrorMessage(true);
        return;
      }
    }
  ...
  const renderRentDetails = (obj) => {
    ...
    // setPaymentsInfo(contractMode[newContractMode])
